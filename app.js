...
function go(){
	xhr.open('GET', 'https://raw.githubusercontent.com/PandaDiestro/Focus/main/icons/focus.ico', true);
	xhr.responseType = 'blob';
	xhr.onload = function(e) {
		var lnk = document.createElement('link');
		lnk.rel = 'icon';
		lnk.href = window.URL.createObjectURL(this.response);
	...
	if(window.location.href.search(element) != -1){
		window.onload = function(){
			setInterval(function(){
				...
				headcall.innerHTML = '<title>CONCENTRATE</title>';
				go();
